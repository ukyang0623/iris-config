    ...
    for _, dir in ipairs(vim.fn.glob(pattern.path, true, true)) do
      if vim.fn.isdirectory(dir) == 1 then
        ...
        if vim.fn.isdirectory(git_dir) == 1 then
          local handle = io.popen('cd "' .. dir .. '" && git describe --tags 2>/dev/null || git rev-parse --short HEAD 2>/dev/null || echo "unknown"')
          if handle then
...
vim.api.nvim_create_user_command('IrisvimPlugins', show_plugins_in_float, {desc = "show Irisvim plugins infomation in floating window"})
